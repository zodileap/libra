...
func (e *ComponentAssetEntity) Config() entity.EntityConfig {
	return entity.EntityConfig{AttrName: "component_asset", Comment: "组件资产"}
}

func (e *ComponentAssetEntity) Fields() []entity.FieldBuilder {
	return []entity.FieldBuilder{
		e.Id.Name("id").Primary(1).Sequence(entity.NewSequence("component_asset_id_seq")).Comment("主键Id"),
		e.OwnerId.Name("owner_id").Comment("所属用户Id"),
		e.Name.Name("name").MaxLen(255).Required().Comment("组件名称"),
