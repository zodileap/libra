...
test("TestProjectSettingsShouldExposeRouteAndSidebarEntry", () => {
  ...
  assert.match(sidebarSource, /<AriPopover/);
  assert.match(sidebarSource, /trigger="click"/);
  assert.match(sidebarSource, /open=\{openWorkspaceActionMenuId === group\.workspace\.id\}/);
  assert.match(sidebarSource, /onOpenChange=\{\(nextOpen\) => \{\s*setOpenWorkspaceActionMenuId\(nextOpen \? group\.workspace\.id : ""\);\s*\}\}/s);
  assert.doesNotMatch(sidebarSource, /workspaceActionPointerGuardRef/);
  assert.doesNotMatch(sidebarSource, /buildWorkspaceActionGuardKey/);
  assert.doesNotMatch(sidebarSource, /handleWorkspaceActionMouseDown/);
  assert.doesNotMatch(sidebarSource, /handleWorkspaceActionClick/);
  assert.doesNotMatch(sidebarSource, /data-workspace-action-trigger/);
  assert.doesNotMatch(sidebarSource, /handleCloseWorkspaceActionMenu/);
