...
test("TestDesktopShouldExposeAdminManagementRoutesInSettings", () => {
  ...
  assert.match(commonRoutesSource, /label: "Overview"/);
  assert.match(commonRoutesSource, /label: "Identities"/);
  assert.match(commonRoutesSource, /label: "Permissions"/);
...
test("TestDesktopBackendApiShouldSupportAdminManagementData", () => {
  ...
  assert.match(overviewSource, /value="快速操作"/);
  assert.match(overviewSource, /value="最近授权"/);
  assert.match(overviewSource, /value="身份上下文"/);
  assert.match(overviewSource, /value="协作者预览"/);
  assert.match(overviewSource, /label="身份管理"/);
  assert.match(overviewSource, /label="权限管理"/);
  assert.match(overviewSource, /label="新增授权"/);
  assert.match(overviewSource, /label="返回工作台"/);
  assert.match(overviewSource, /label="查看记录"/);
  assert.match(overviewSource, /label="授权给他"/);
  ...
  assert.match(overviewSource, /formatGrantTime\(grant\.createdAt \|\| grant\.lastAt \|\| ""\)/);
  ...
  assert.match(identitiesSource, /当前身份：/);
  assert.match(identitiesSource, /label=\{selectedIdentity\?\.id === item\.id \? "当前身份" : "设为当前"\}/);
  assert.match(settingsGeneralSource, /selectedIdentity: ConsoleIdentityItem \| null;/);
  assert.match(settingsGeneralSource, /title="Current Identity"/);
  assert.match(settingsGeneralSource, /value=\{selectedIdentity\?\.scopeName \|\| "未选定身份"\}/);
  ...
  assert.match(permissionsSource, /label="目标用户名称"/);
  assert.match(permissionsSource, /label="目标用户邮箱"/);
