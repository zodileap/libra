...
test("TestSessionCopyShouldBeMovedToDevDebugPanel", () => {
  ...
  assert.match(sessionSource, /function normalizeRunSegmentIntroForCopy\(intro: string, step: string\): string \{/);
  ...
  assert.match(sessionSource, /shouldHideRunSegmentInTimeline\(segment\.intro, segment\.step, segment\.status\)/);
  ...
  assert.match(sessionSource, /const renderableTimeline = resolveRenderableAssistantRunTimeline\(message\.text, runMeta\);/);
  assert.match(sessionSource, /const visibleTimeline = resolveVisibleAssistantRunTimeline\(/);
  assert.match(sessionSource, /const visibleTimelineItems = visibleTimeline\.map\(\(item\) => \{/);
  ...
  assert.match(sessionSource, /const visibleRunSummaryText = resolveVisibleCompletedRunSummaryText\(normalizedMessageText,\s*runMeta\);/);
  assert.match(sessionSource, /visible_timeline: visibleTimelineItems,/);
  assert.match(sessionSource, /running_indicator:/);
  assert.doesNotMatch(sessionSource, /visible_summary:/);
