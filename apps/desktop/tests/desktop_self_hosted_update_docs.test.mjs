...
test("TestDesktopSelfHostedUpdateDocsShouldDescribeOfficialUpdaterFlow", () => {
  ...
  const releaseScriptSource = readRepoSource("scripts/package-desktop-release.mjs");
  const windowsWrapperSource = readRepoSource("scripts/package-desktop-release.cmd");
  ...
  assert.match(rootReadme, /scripts\\package-desktop-release\.cmd 0\.1\.1/);
  ...
  assert.match(docSource, /scripts\\package-desktop-release\.cmd 0\.1\.1/);
  ...
  assert.match(docSource, /pnpm -C \/Users\/yoho\/code\/libra\/apps\/desktop build/);
