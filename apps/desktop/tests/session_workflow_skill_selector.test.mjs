...
test("TestSessionPageShouldProvideWorkflowAndSkillSelectorModal", () => {
  ...
  assert.match(sessionSource, /setStreamingAssistantTarget\(heartbeatText\);/);
  ...
  assert.match(sessionSource, /const meaningfulSegments = sanitizedSegments\.filter\(\(item\) => !isThinkingPlaceholderSegment\(item\)\);/);
  assert.match(sessionSource, /const latestThinkingSegment = \[\.\.\.sanitizedSegments\]\.reverse\(\)\.find\(\(item\) => isThinkingPlaceholderSegment\(item\)\);/);
  ...
  assert.match(sessionSource, /\.filter\(\(group\) => group\.steps\.length > 0 \|\| group\.title !== "执行过程"\);/);
  ...
  assert.match(sessionSource, /已等待约 \$\{waitedSeconds\} 秒/);
