...
test("TestSessionPageShouldProvideWorkflowAndSkillSelectorModal", () => {
  ...
  assert.match(sessionSource, /buildSessionContextPrompt\(\s*messages,\s*normalizedContent,\s*undefined,\s*latestProjectProfile,\s*activeWorkspaceEnabledCapabilities,\s*\)/s);
  assert.match(sessionSource, /buildSessionContextPrompt\(\s*contextMessages,\s*normalizedContent,\s*String\(activeWorkspace\?\.path \|\| ""\)\.trim\(\) \|\| undefined,\s*latestProjectProfile,\s*activeWorkspaceEnabledCapabilities,\s*\)/s);
  assert.match(sessionSource, /const missingCapabilityLabels = selectedWorkflowMissingRequiredCapabilities/);
  assert.match(sessionSource, /AriMessage\.error\(t\("当前工作流缺少必需的项目能力：\{\{capabilities\}\}", \{\s*capabilities: missingCapabilityLabels\.join\("、"\),\s*\}\)\);/s);
