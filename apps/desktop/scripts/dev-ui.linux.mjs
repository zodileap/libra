...
// 描述：
//
//   - 组装 Linux 平台专用的 Desktop UI 开发脚本运行配置，保留单独脚本入口，便于后续追加系统差异。
//
// Returns:
//
//   - systemName: 系统标识。
...
//   - killPid: 进程结束函数。
export function resolveLinuxDevUiRuntime() {
  return {
    systemName: "linux",
    pnpmCommand: "pnpm",
    useShell: false,
...
// 描述：
//
//   - 运行 Linux 平台专用的 Desktop UI 开发脚本。
export async function main() {
  await runDevUi(resolveLinuxDevUiRuntime());
}

if (process.argv[1] && path.resolve(process.argv[1]) === CURRENT_FILE_PATH) {
  main().catch((err) => {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[dev:ui:linux] ${message}`);
    process.exit(1);
  });
}
